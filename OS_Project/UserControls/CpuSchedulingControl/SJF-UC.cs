...
{
    public partial class SJF_UC : UserControl
    {
        public SJF_UC()
        {
            InitializeComponent();
        ...
        private void BtnStartScheduling_Click(object sender, EventArgs e)
        {
            ...
            if (!ProDGV.Columns.Contains("PStartTime"))
            {
                ProDGV.Columns.Add("PWaitTime", "Waiting");
                ProDGV.Columns.Add("PTurnAroundTime", "Turnaround");
            }

            var scheduler = new SJFScheduler(processList , CheckBoxPreemptive.Checked);
            scheduler.Run();

            // Cập nhật lại processList từ ResultProcesses
            processList = scheduler.ResultProcesses;
            var drawer = new GanttChartDrawer(pnlGanttContainer);
            drawer.Draw(processList);
